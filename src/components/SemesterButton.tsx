...
export default function SemesterButton() {
    ...
    const semesterLimit = 9;

    const [semester, setSemester] = useState<Array<JSX.Element>>([<SemesterElement count={1} />]);
    const [semesterCount, setSemesterCount] = useState<number>(2);

    const handleNewSemester = (e: React.MouseEvent<HTMLButtonElement>) => {
        e.preventDefault();

        if (semesterCount < semesterLimit) {
            setSemester((prevSemester) => [...prevSemester, <SemesterElement key={semesterCount} count={semesterCount} />]);
            setSemesterCount((prevCount) => prevCount + 1);
        } else {
            alert('Vous avez atteint le nombre maximal de semestres.');
        }
    };
